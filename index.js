...
function render(contacts) {
  const contactsElm = document.getElementById("contacts");
  contactsElm.innerHTML = "";
  const elm = contacts.map(renderContact).join("");
  contactsElm.innerHTML = elm;
   }
...
function filterByCity(city) {
  ...
  const filteredCity = contacts.filter(contact=> contact.address.city === city);
...
function filterHandler(e) {
 ...
 if(e.target.value=="0"){
   ...
   const filter=filterByCity(e.target.value)
   render(filter)
 }
}
...
function loadCities(contacts) {
  const cities= contacts.map(c=> c.address.city)
  const uniqueCities= cities.filter((city, index, self) => self.indexOf(city) === index);
  
  const filterOptions=document.getElementById('filterOptions')
  let html='<option value="0">-- Select a city --</option>';
 
  for(let i=0;i<uniqueCities.length;i++){
    html=html+
   `<option value="${uniqueCities[i]}">${uniqueCities[i]}</option>`
  }
...
*/
function main() {
  document.getElementById('filterOptions').addEventListener("change",filterHandler)
   loadCities(contacts)
   render(contacts)
}
main()
